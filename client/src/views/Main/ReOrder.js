...
const ReOrder = ({ match, history }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      try {
        ...
        shippingAddress = shipping.find((type) => type.name === "Personalizado")
          .id;
        setSelectedShipping(value.id);
        setShippingTypes(shipping);
        const payment = await getPaymentTypes();
        const payValue = payment.find(
          (type) => type.description === paymentMethod
        );
  ...
  const sendRequest = () => {
    ...
    const payment = paymentMethods.find(
      (type) => type.idPaymentMethods === Number(selectedPayment)
    );
    ...
    createRequest(requestInfo, 1)
      .then(async (res) => {
        ...
        if (res.code === 1) {
          ...
          await Swal.fire(
            "Pedido creado",
            "Tu pedido se ha creado procede con el pago",
            "success"
          );
  ...
  if (loading) {
    ...
    return (
      <div className="row p-5">
        ...
        <div className="col-lg-7 col-sm-12 mt-3 mb-3">
          <div className="row">
          ...
          <div className="d-flex flex-row align-items-center justify-content-center Left mb-4">
            ...
            >
              {paymentMethods.map((type) => (
                <option
                  key={type.idPaymentMethods}
                  value={type.idPaymentMethods}
                >
                  {type.description}
                </option>
              ))}
            ...
            {paypalButton && paypalURL !== "" ? (
              ...
              >
                {loadingButton ? (
                  ...
                  <>
                    <MdCheckCircle className="text-white mr-1" /> Realizar
                    pedido
                  </>
                )}
              </button>
